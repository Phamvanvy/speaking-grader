...
def _tally_points(points: list, tallies: dict[str, dict], weight: float) -> None:
    """Cộng 1 danh sách PhonemePoint dict vào tallies (key = normalize_ipa)."""
    for p in points:
        ...
        if status == "sub" and err > 0 and p.get("heard"):
            heard_key = normalize_ipa(str(p["heard"]))
            t["heard"][heard_key] = t["heard"].get(heard_key, 0) + weight


def _tally_result(result: dict, tallies: dict[str, dict], weight: float = 1.0) -> None:
...
def refresh_profile(cfg: Config, user_id: str) -> None:
    ...
    for r in results:
        _tally_result(r["result"], tallies, weight=_recency_weight(r["created_at"], now))
    # Apply cả khi tallies rỗng (blob không có phoneme) để con trỏ vẫn tiến.
    words.apply_phoneme_tallies(cfg, user_id, tallies, cursor)


def get_weak_phonemes(
    cfg: Config, user_id: str, top_k: int = 5
) -> tuple[list[dict], str]:
    """Top âm yếu của user: (danh sách weak-item, source "history"/"fallback").
    ...
    stats = words.get_phoneme_stats(cfg, user_id)
    tallies: dict[str, dict] = {
    ...
    for entry in words.list_words(cfg, user_id)["words"]:
        _tally_points(entry.get("phonemes") or [], tallies, _SAVED_SNAPSHOT_WEIGHT)
    ...
    if len(weak) < 2:
        ...
        for sym in FALLBACK_WEAK_PHONEMES:
            if len(weak) >= top_k:
                break
            if normalize_ipa(sym) in have:
                continue
            weak.append({
                "symbol": normalize_ipa(sym), "attempts": 0, "errors": 0,
