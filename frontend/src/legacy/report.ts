// @ts-nocheck
// Legacy print/download helpers (port từ web/js/report.js) — dùng chung cho print
// PDF (single/batch/exam) và tải audio. Verbatim; escapeHtml import từ lib.

import { escapeHtml } from '../lib/format';
import { examConfig } from '../lib/config';
import { featureTiles, phonemeErrorsHtml, itemProcessingMs, fmtMs } from './render';
...
// Tải nhiều file liên tiếp: trình duyệt chặn nhiều lượt tải sát nhau → giãn cách nhẹ.
export async function downloadBlobsSequentially(entries) {
...
export function reportCriteriaHtml(scores, cfg) {
...
// Mở cửa sổ in với HTML dựng sẵn (dùng chung cho single/batch/exam print).
...
// ── Print single (port printSingleReport; data + filename truyền vào thay global) ──
export function printSingleReport(data, filename) {
  if (!data) {
  ...
  const fn = data.audio_filename || filename || 'recording';
...
<title>${escapeHtml(cfg.label)} Speaking Report — ${escapeHtml(fn)}</title>
<style>${reportStyles()}</style></head>
<body>
  <h1>${escapeHtml(cfg.label)} Speaking Report</h1>
  <div class="meta">File: ${escapeHtml(fn)} · Generated ${escapeHtml(new Date().toLocaleString())}</div>
  ${
  ...
  ${phonemeErrorsHtml(data.phoneme)}
...
// ── Print batch (port printBatchReport) ──
export function printBatchReport(data) {
