...
// Tip cấu âm tiếng Việt cho từng phoneme (kiểu ELSA) + từ ví dụ để phát mẫu TTS.
// Dùng trong popup luyện từ (practice.js): âm sai → hiện tip; nút 🔊 mỗi âm phát
// TTS của TỪ VÍ DỤ qua /tts (Piper không tổng hợp được phoneme cô lập — xem
// src/tts.py). Key là symbol ĐÃ chuẩn hoá của backend (US, có ː); lookup trong
// phonemeTip() tự thử thêm biến thể bỏ ː / GB (əʊ→oʊ, ɜː→ɝ).
const PHONEME_TIPS = {
    // ── Phụ âm ──
...
// Tra tip cho 1 symbol: thử nguyên bản → bỏ/thêm ː → biến thể ngắn. Trả
// {tip, example} hoặc null (symbol lạ → caller hiện fallback, không có nút 🔊).
function phonemeTip(symbol) {
    if (!symbol) return null;
    const s = String(symbol);
    return PHONEME_TIPS[s]
        || PHONEME_TIPS[s + 'ː']
        || PHONEME_TIPS[s.replace(/ː$/, '')]
        || null;
}
